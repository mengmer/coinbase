    ...
    def main(self):
        ...
        for symbol in self.symbol_config.keys():
            ...
            symbol_candle_data[symbol] = fetch_okex_symbol_history_candle_data(
                self.symbol_config[symbol]['instrument_id'], self.time_interval, 1000, max_try_amount=5)
        ...
        while True:
            ...
            symbol_info = pd.DataFrame(index=symbol_config.keys(),
                                       columns=symbol_info_columns)  # 转化为dataframe , index=symbol_config.keys(),columns=symbol_info_columns
            ...
            dingding_report_every_loop(symbol_info, symbol_signal, symbol_order, run_time, ['xxxx', 'xxx'])
...
if __name__ == '__main__':
    ...
    symbol_config = {
        'eth-usdt': {'instrument_id': 'ETH-USDT-220325',  # 合约代码，当更换合约的时候需要手工修改
                     'leverage': '2',  # 控制实际交易的杠杆倍数，在实际交易中可以自己修改。此处杠杆数，必须小于页面上的最大杠杆数限制
                     'strategy_name': 'bolling_new_ema_pingjuncha_stopEarly_stopAtReverse',  # 使用的策略的名称
                     'para': [80, 2.8, 0.06, 5.6]},  # 策略参数 [470,2.9,0.13]
        # 'btc-usdt': {'instrument_id': 'BTC-USDT-210625',  # 合约代码，当更换合约的时候需要手工修改
        #              'leverage': '1',  # 控制实际交易的杠杆倍数，在实际交易中可以自己修改。此处杠杆数，必须小于页面上的最大杠杆数限制
