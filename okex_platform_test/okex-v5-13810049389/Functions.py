...
def fetch_future_account(max_try_amount=5):
    ...
    for _ in range(max_try_amount):
        try:
            ...
            future_info = \
            session.get(getFuturePositionUrl, headers=getPrivateHeaders('/api/v5/account/balance?ccy=USDT', 'GET'),
                        timeout=3).json()['data']
            ...
            for ccyMsg in future_info[0].get('details'):
                if ccyMsg.get('ccy') == 'USDT':
                    # 返回两个值，账户总权益和可用保证金
                    return float(ccyMsg.get('eq', 0)), float(ccyMsg.get('availEq', 0))
            # 如果没查询到那么就返回0
            return 0, 0
...
def single_threading_get_data(symbol_info, symbol_config, time_interval, run_time, candle_num, max_try_amount=5):
    ...
    # 逐个获取symbol对应的K线数据
    for symbol in symbol_config.keys():
        _, symbol_candle_data[symbol], symbol_info.at[symbol, '信号价格'] = get_candle_data(symbol_config, time_interval,
                                                                                        run_time, max_try_amount,
                                                                                        candle_num, symbol)
...
def single_threading_place_order(symbol_info, symbol_config, symbol_signal, max_try_amount=5):
    ...
    if symbol_signal:
        ...
        for symbol in symbol_signal.keys():
            # 下单
            _, order_id_list = okex_future_place_order(symbol_info, symbol_config, symbol_signal, max_try_amount,
                                                       symbol)
        ...
        for i in range(max_try_amount):
            try:
                ...
                if len(symbol_signal[symbol]) == 2 and order_type in [1, 2]:  # 当两个条件同时满足时，说明当前处于平仓后，需要再开仓的阶段。
                    time.sleep(medium_sleep_time)  # 短暂的休息2s，防止之平仓后，账户没有更新
                    _, availEq = fetch_future_account()
                    symbol_info.at[symbol, "账户权益"] = availEq
                    symbol_info.at[symbol, "持仓量"] = numpy.NaN
                ...
                if order_type == 1:  # 开多
                    params['side'] = 'buy'
                    params['posSide'] = 'long'
                    # symbol_config[symbol]['leverage']='3'
                if order_type == 2:  # 开空
                    params['side'] = 'sell'
                    params['posSide'] = 'short'
                    # symbol_config[symbol]['leverage'] = '1'
                if order_type == 3:  # 平多
                    params['side'] = 'sell'
                    params['posSide'] = 'long'
                if order_type == 4:  # 平空
                ...
                if update_price_flag:
                    ...
                    #     ]
                    # }
                    limitPriceUrl = baseUrl + 'api/v5/public/price-limit?instId=' + symbol_config[symbol][
                        'instrument_id']
                ...
                order_info = tradeAPI.place_order(instId=params['instId'], tdMode=params['tdMode'], side=params['side'],
                                                  posSide=params['posSide'],
                                                  ordType=params['ordType'], sz=params['sz'], px=params['px'])['data'][
                    0]
        ...
        for order_id in symbol_order.index:
            ...
            for i in range(max_try_amount):
                try:
                    ...
                    #     'order_id': order_id
                    # }
                    rightPart = '/api/v5/trade/order?ordId={ordId}&instId={instId}'.format(ordId=order_id, instId=
                    symbol_config[symbol_order.at[order_id, 'symbol']]["instrument_id"])
                    getOrderDetailUrl = baseUrl[:-1] + rightPart
                    order_info = \
                    session.get(getOrderDetailUrl, headers=getPrivateHeaders(rightPart, 'GET'), timeout=3).json()[
                        'data'][0]
                    break
                except Exception as e:
                    print(e)
...
def dingding_report_every_loop(symbol_info, symbol_signal, symbol_order, run_time, robot_id_secret):
    ...
    if run_time.minute % 60 == 0 or symbol_signal:
...
# 发送钉钉消息
def send_dingding_msg(content, robot_id='5937300fe0fd347e26a17de806c4b26908371fc24cbb53fbc8a5aaebd0bc1e8b',
                      secret='SEC55cc6141bb0947dd5272301bbeb54f373eb43b9955aee142408e6bc14a97c753'):
    """
    :param content:
...
n    :param secret: 你的secret，即安全设置加签当中的那个密钥
    ...
    try:
        msg = {
            "msgtype": "text",
            "text": {"content": content_prefix + content + '\n' + datetime.datetime.now().strftime("%m-%d %H:%M:%S")}}
