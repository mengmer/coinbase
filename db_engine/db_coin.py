...
def get_coin_his_data(exchange, symbol, time_interval, start_time=None, end_time=None, path=None):
    ...
    while True:
        ...
        if exchange.name == 'OKEX':
            ...
            # 整理数据
            df = pd.DataFrame(df)  # 将数据转换为dataframe
...
def get_coin_now_data(exchange, symbol, time_interval='1m'):
    """
    获取实时数据
    """
    # ===对火币的limit做特殊处理
...
def get_all_coin_his_data(start_time=None, end_time=None):
    """
    获取所有数据
    """
    if start_time is None:
...
if __name__ == '__main__':
    ...
    sy_list = ['BTC/USDT', 'ETH/USDT', 'EOS/USDT', 'LTC/USDT', 'DOGE/USDT']
    k_line = '15m'  # 其他可以尝试的值：'1m', '5m', '15m', '30m', '1H', '2H', '1D', '1W', '1M', '1Y'
    start = '2018-01-01 00:00:00'
    end = '2022-09-09 00:00:00'
    # 作业1： 抓取币安历史数据
    # get_coins_his_data(exchange=ex_ba, symbol_list=sy_list, time_interval=k_line,
    #                    start_time=start, end_time=end,
    #                    path=r"D:\coinbase\data")
    # 作业2：实时收集okx数据
    x = get_coin_now_data(ex_okx, 'BTC/USDT')
